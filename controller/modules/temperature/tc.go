...
func (c *Controller) Create(tc *TC) error {
    ...
    c.tcs[tc.ID] = tc
    if err := c.statsMgr.Initialize(tc.ID); err != nil {
        return err
    }
    if tc.Enable {
        ...
        go func() {
            err := c.Run(tc, quit)
            if err != nil {
                log.Println("ERROR: temperature subsystem. Failed to run sensor. Error:", err)
            }
        }()
    }
    return nil
...
func (c *Controller) Delete(id string) error {
    ...
    if deleteCalibration {
        err := c.c.Store().Delete(CalibrationBucket, tc.Sensor)
        if err != nil {
            log.Println("ERROR: temperature sub-system: Failed to delete calibration details for sensor:", tc.Sensor)
        }
...
func (c *Controller) Run(t *TC, quit chan struct{}) error {
    ...
    log.Println("temperature sub-system: Starting sensor:", t.Name)
    t.ticker = time.NewTicker(t.Period * time.Second)
    ...
    for {
        ...
        case <-quit:
            log.Println("temperature sub-system: Stopping sensor:", t.Name)
            t.ticker.Stop()
            log.Println("temperature sub-system: Stopped sensor:", t.Name)
            return nil
        }
    }
}
